...
var DailyDemand = new cLASS({
  ...
  methods: {
    "onEvent": function () {
	  ...
	  if (sQ > 0) {
		   tQ +=sQ - this.quantity,
       sim.stat.servicelevel = (tQ/(tQ+sim.stat.lostSales))*100;
		 }
	 if (sQ > 0) {
	 	 sim.stat.totalcost +=sQ*sH+rP*oC+sim.stat.lostSales*sOC;
	 }
	 if (sim.v.replenishmentPolicy === 1){
    ...
    } else return [new Delivery({
          occTime: this.occTime + Delivery.leadTime()+ this.reorderInterval,
         quantity: sh.targetInventory - newSQ,
          receiver: sh
        })]; 
},
